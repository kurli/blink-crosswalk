...
void addBuffer( vector<Scene::GLES2Object>& objlist, BufferGeometryRef buffer, Object3DRef object )
{
    ...
    LOGD("Adding GLES2Object buffer %p object %p\n", &buffer, object.get());
            ...
            for ( auto& offset : offsets ) {
                ...
                if ( updateBuffers ) {
                    ...
                    for ( auto& kv : program->attributes ) {
                        ...
                        LOGD("Update buffers: name %s attrib pointer %d item %p\n",
...
GLuint GLES2Renderer::getTextureUnit()
{
    ...
    if ( textureUnit >= _maxTextures ) {
        ...
        LOGD( "WebGLRenderer: trying to use %d texture units while this GPU supports only %d\n",
    ...
    for ( auto& kv : uniforms ) {
        ...
        if ( type == 'i' ) { // single integer
            ...
            if (value.size() > 0) {
                ...
                LOGD("Updated fv array location %d size %d pointer %p [0] (%f,%f,%f)\n",
            ...
            */
            ...
            LOGD( "RESIN::GLES2Renderer: Unknown uniform type: %x", type );
...
//    };
//
    LOGD("material->vertexShader: %s\n", material->vertexShader().c_str());
...
GLuint getShader ( const string& type, const string& str )
{
    ...
    if (success != GL_TRUE) {
       ...
       if ( length ) {
          ...
          LOGD("Shader %d error: %s", shader, buffer);
          delete [] buffer;
        }
    }
    else {
        LOGD( "Shader %d compiled OK\n", shader );
    }
...
void cacheUniformLocations ( Program& program, vector<string>& identifiers )
{
    for (auto& id : identifiers) {
        ...
        LOGD("Caching uniform identifier %s location %d\n", id.c_str(), program.uniforms[id]);
    }
}
...
void cacheAttributeLocations ( Program& program, vector<string>& identifiers )
{
    for (auto& id : identifiers) {
        ...
        LOGD("Caching attrib identifier %s location %d\n", id.c_str(), program.attributes[id]);
    }
}
...
void GLES2Renderer::setClearColor(const Color& color, float alpha)
{
    ...
    LOGD("set clear color %f %f %f %f",  _clearColor.r, _clearColor.g, _clearColor.b, _clearAlpha );
...
void GLES2Renderer::initGLES2Objects(SceneRef scene)
{
    ...
    for ( auto go : scene->__gles2Objects ) {
        LOGD("Iterating GLES2Object buffer %p object %p\n", &(go.buffer), &(go.object));
